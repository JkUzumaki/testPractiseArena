...
class Rectangle {
   public:
   Rectangle(int height, int width, int length){ cout<<"hi"<<endl;}
};
class Traingle {
   public:
   Traingle(int height, int width, int length){ cout<<"hi2"<<endl;}
};

void draw_rect(Traingle r){};
...
class Traingle {
   public:
   Traingle(int height, int width, int length){ cout<<"hi2"<<endl;}
   Traingle(int height, int width){ cout<<"hi3"<<endl;}
};

void draw_rect(Traingle r){};

int main() {
   draw_rect({5, 6});  // Rectangle{5,6,9} is automatically called
}
...
for (auto i: v) {
   ...
   for (auto i: v) {
      ...
      std::cout << i << std::endl;// changes the values in v
   }
...
/* Cons:
 * 1. Cumbersome(large or heavy and therefore difficult to carry or use) code.
...
class Dog {
   ...
   int age = 9;
   public:
   Dog() {std::cout << "hi" << std::endl; }
   Dog(int a) : Dog() {: Dog() std::cout << "hello" << std::endl; }
};
int main(){
